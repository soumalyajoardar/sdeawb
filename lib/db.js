...
export function saveDatabase(data) {
  ...
  if (supabase) {
    const collections = [
      ...
      { name: 'sdea_legacy_members', data: data.legacyMembers || [] },
...
// Legacy Members
export function getLegacyMembers() {
  const db = getDatabase();
  return db.legacyMembers || [];
}

export function createLegacyMember(data) {
  const db = getDatabase();
  const newMember = {
    id: 'leg-' + Date.now(),
    membershipId: (data.membershipId || '').trim(),
    entryYear: data.entryYear || '',
    paidUpYear: data.paidUpYear || '',
    fullName: (data.fullName || '').trim(),
    designation: data.designation || 'Sub-Assistant Engineer',
    department: data.department || '',
    postingOffice: data.postingOffice || '',
    district: data.district || '',
    hrmsId: data.hrmsId || '',
  ...
  db.legacyMembers = [newMember, ...(db.legacyMembers || [])];
  saveDatabase(db);
  return newMember;
}

export function updateLegacyMember(id, updates) {
  const db = getDatabase();
  db.legacyMembers = (db.legacyMembers || []).map(m => 
    m.id === id ? { ...m, ...updates } : m
  );
  saveDatabase(db);
  return db.legacyMembers.find(m => m.id === id);
}

export function deleteLegacyMember(id) {
  const db = getDatabase();
  db.legacyMembers = (db.legacyMembers || []).filter(m => m.id !== id);
  saveDatabase(db);
  return true;
}
